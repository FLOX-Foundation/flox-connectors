...
BybitOrderExecutor::BybitOrderExecutor(std::unique_ptr<AuthenticatedRestClient> client, SymbolRegistry* registry, OrderTracker* orderTracker)
...
void BybitOrderExecutor::submitOrder(const Order& order)
{
  ...
  body.append("\"qty\":\"").append(std::to_string(order.quantity.toDouble())).append("\",");
  body.append("\"price\":\"").append(std::to_string(order.price.toDouble())).append("\"}");

  FLOX_LOG("[BybitOrderExecutor] Submitting order: id="
           << order.id << " symbol=" << info->symbol
           << " side=" << (order.side == Side::BUY ? "Buy" : "Sell")
           << " qty=" << order.quantity.toDouble()
           << " price=" << order.price.toDouble()
           << " category=" << Bybit::toString(info->type));

  _client->post("/v5/order/create", body, [this, order](std::string_view response)
                {
                  ...
                  if (retCode != 0)
                  {
                    ...
                    FLOX_LOG_ERROR("[BybitOrderExecutor] Order submission failed: retCode=" << retCode
                                                                                            << " retMsg=" << retMsg);
                    return;
                  }
                  ...
                  _orderTracker->onSubmitted(order, orderId); }, [this](std::string_view error)
...
void BybitOrderExecutor::cancelOrder(OrderId orderId)
{
  ...
  FLOX_LOG_INFO("[BybitOrderExecutor] Cancelling order: localId=" << orderId
                                                                  << " exchangeId=" << exchangeOrderId);

  _client->post("/v5/order/cancel", body, [this, orderId](std::string_view response)
                {
      simdjson::ondemand::parser parser;
      simdjson::padded_string padded(response);
      auto doc = parser.iterate(padded);
      ...
      if (retCode != 0) {
        ...
        FLOX_LOG_ERROR("[BybitOrderExecutor] Cancel failed: orderId=" << orderId
                         << " retCode=" << retCode << " msg=" << msg);
      } else {
        ...
        _orderTracker->onCanceled(orderId);
                ...
                { FLOX_LOG_ERROR("[BybitOrderExecutor] Cancel transport error: orderId=" << orderId << " err=" << err); });
}
...
void BybitOrderExecutor::replaceOrder(OrderId oldOrderId, const Order& newOrder)
{
  ...
  std::string qty = std::to_string(newOrder.quantity.toDouble());
  std::string price = std::to_string(newOrder.price.toDouble());
  ...
  FLOX_LOG_INFO("[BybitOrderExecutor] Replacing order: localId=" << oldOrderId
                                                                 << " exchangeId=" << exchangeOrderId
                                                                 << " newQty=" << qty
                                                                 << " newPrice=" << price);

  _client->post("/v5/order/amend", body, [this, oldOrderId, newOrder](std::string_view response)
                {
      simdjson::ondemand::parser parser;
      simdjson::padded_string padded(response);
      auto doc = parser.iterate(padded);
      ...
      if (retCode != 0) {
        std::string_view msg = doc["retMsg"].get_string().value();
        FLOX_LOG_ERROR("[BybitOrderExecutor] Replace failed: orderId=" << oldOrderId
                         << " retCode=" << retCode << " msg=" << msg);
      } else {
        ...
        _orderTracker->onReplaced(oldOrderId, newOrder, "");
                ...
                { FLOX_LOG_ERROR("[BybitOrderExecutor] Replace transport error: orderId=" << oldOrderId << " err=" << err); });
}
